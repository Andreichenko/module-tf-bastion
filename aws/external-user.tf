...
data "template_file" "additional_user" {
  ...
  template = <<EOF
info "Creating user:"
# The printf string is put in single-quotes because it may contain its own double-quotes.
printf '  Login: \"$${user_login}\"\n'

# The ssh_keys variable is put in single-quotes because it may contain its own double-quotes.
